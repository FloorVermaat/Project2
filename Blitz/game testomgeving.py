...
def draw_text(surf, text, size, x, y):
    font_name = pygame.font.Font("8-BIT WONDER.ttf", size)
...
def Hubworld():
    global nameinputscreen, hubworldstatus
    waiting = True
    while waiting:
...
class BlitzWorld(pygame.sprite.Sprite):
    def __init__(self):
        pygame.sprite.Sprite.__init__(self)
        self.image_orig = blitzplanet_image
        self.image = blitzplanet_image
...
class BossShip(pygame.sprite.Sprite):
    ...
    def shoot(self):
        ...
        if now - self.last_update > self.shoot_delay:
            self.last_update = now
            bullet = EnemyBullet(self.rect.centerx, self.rect.bottom, +10)
            all_sprites.add(bullet)
            enemybullets.add(bullet)
...
# planet images
blitzplanet_image = spacemine_image_orig = pygame.image.load(os.path.join(img_folder, "blitz planet.png")).convert_alpha()
...
while not done:
    ...
    if score >= enemyfleet.enemyspawn + 700 and not bossbattle:
        for i in range(1):
    ...
    # BOSS BATTLE!
    if score >= 50000 and not bossbattle:
